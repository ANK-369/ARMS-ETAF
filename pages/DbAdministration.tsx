...
const DbAdministration: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (pendingKey !== null) {
            ...
            const savedDbKey = getDB().securityCredentials?.secretKey || getDB().secretKey || '';
            setSecretKey(sessionKey || savedDbKey);
    ...
    const handleSaveSecretKey = async () => {
        ...
        if (wasGenerated) {
            ...
            // Just treat as temporary session key. Save to sessionStorage and arms_readonly_secret_key for this session.
            if (trimmed) {
            ...
            setMsg(trimmed
                ? (language === 'en' ? "Secret Key saved as temporary session key!" : "የሚስጥር ቁልፍ እንደ ጊዜያዊ ፈቃድ ቁልፍ ተቀምጧል!")
                : (language === 'en' ? "Secret Key cleared." : "የሚስጥር ቁልፍ ተሰርዟል።")
            );
            setError(false);
        }
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 p-6 md:p-12 overflow-y-auto h-full pb-32">
            {/* Same DB Admin UI as before */}
            <h3 className="text-3xl font-bold text-gold-500 border-b border-gray-700 pb-4 mb-8 flex items-center gap-3"><Database size={32}/> {t('dbAdministration')}</h3>
