...
const Store: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const currentDay = parseInt(currD) || daysInMonth;
    ...
    const activeManpower = db.manpower.filter(m => {
        ...
        const mEnd = `${selectedYear}-${selectedMonth}-${daysInMonth.toString().padStart(2, '0')}`;
        ...
        if (sY === selectedYear && sM === selectedMonth) {
            ...
            startDay = daysInMonth + 1; // Starts after this month? (Already filtered by overlap, but safety)
        }
  ...
  const DateSelectors = ({ form, setForm }: any) => {
      ...
      const handleMonthChange = (val: string) => {
          ...
          const newMax = getDaysInEthiopianMonth(year, newM);
          let newDay = form.day;
